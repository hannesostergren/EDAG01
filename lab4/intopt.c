...
node_t* initial_node(int m, int n, double** a, double* b, double* c){
    ...
    p->m = m;
    p->n = n;
    memcpy(p->a, a, sizeof(a)); //vet inte om detta funkar
    memcpy(p->b, b, sizeof(b));
    memcpy(p->c, c, sizeof(c));
...
node_t* extend(node_t* p, int m, int n, double** a, double* b, double* c, int k, double ak, double bk){
    ...
    for(i = 0; i < q->m+1; i += 1){
        q->a[i] = calloc(q->n+1, sizeof(*q->a[i]));
    }
...
int bound(node_t* p, node_t* h, double* zp, double* x){
    node_t* q;
    if (p->z > *zp){
        *zp = p->z;
        x = p->x; //how do i save best x 
        if(q->z < p->z){
            free(h); //free(q)?
        }
    }
